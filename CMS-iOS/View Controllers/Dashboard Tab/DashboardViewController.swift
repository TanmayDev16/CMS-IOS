...
//        isLoading = false
    ...
    func tryToUneroll(courseId: Int, indexPath: IndexPath) {
        ...
        CourseUnenroller.shared.attemptUnenroll(courseId: courseId, userId: userDetails.userid) {
            showLoginAlert, completed in
            self.gradientLoadingBar.fadeOut()
            if showLoginAlert {
                let alert = UIAlertController(title: "Unavailable", message: "This feature is unavailable because the required token is not found. Please logout and log back in with Google to enable this feature.", preferredStyle: .alert)
                ...
                if completed {
                    ...
                    let banner = NotificationBanner(title: "Failed", subtitle: "Error unenrolling", style: .danger)
                    banner.show()
                }
            }
        }
